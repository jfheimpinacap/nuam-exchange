...
export function TaxClassificationApiForm({ mode, id, initialValues }: Props) {
  ...
  const navigationState = location.state as { taxClassificationWriteSuccess?: string } | null;
  const [writeSuccess] = useState(navigationState?.taxClassificationWriteSuccess ?? '');
  const refs = useRef<Partial<Record<keyof Values, HTMLInputElement | HTMLTextAreaElement>>>({});
  useEffect(() => { if (writeSuccess) navigate(location.pathname, { replace: true }); }, [location.pathname, navigate, writeSuccess]);
