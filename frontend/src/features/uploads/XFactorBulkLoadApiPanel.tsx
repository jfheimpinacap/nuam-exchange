...
import type { BulkLoadXFactorResultDto, TaxClassificationsBulkLoadService } from '../../api/services/TaxClassificationsBulkLoadService';
import { Button } from '../../components/Button';
import { InlineMessage } from '../../components/InlineMessage';
import type { UserRole } from '../../types/session';

const REQUIRED_HEADER = 'market;instrumentCode;taxPeriod;appliedFactor';
...
export function XFactorBulkLoadApiPanel({ role, service }: { role: UserRole | undefined; service: TaxClassificationsBulkLoadService | null }) {
  const [file, setFile] = useState<File | null>(null);
  const [stage, setStage] = useState<ApiStage>('selection');
  const [error, setError] = useState('');
  const [success, setSuccess] = useState('');
  const [result, setResult] = useState<BulkLoadXFactorResultDto | null>(null);
  ...
  const submit = async () => {
    ...
    setSuccess('');
    try {
      const response = await service.uploadXFactor(file, controller.signal);
      if (controller.signal.aborted) return;
      setResult(response);
      setSuccess('Carga masiva X Factor procesada correctamente.');
      setStage('completed');
  ...
  if (!isAuthorized(role)) {
    return <InlineMessage tone="warning" message="No posee permisos para realizar cargas masivas. Solo Administrador y Analista Tributario pueden ejecutar la carga real de X Factor." />;
  }
