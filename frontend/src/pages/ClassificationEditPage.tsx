...
import { valuesFromTaxClassificationDetail } from '../features/classifications/taxClassificationApiFormValues';
...
export function ClassificationEditPage() {
  ...
  if (isApi) {
    ...
    if (!detail || !Number.isInteger(numericId)) return <section className="content-card not-found-panel"><h1>Registro no encontrado</h1><p>No existe una calificación tributaria con el identificador solicitado.</p><Button variant="primary" onClick={() => navigate('/calificaciones')}>Volver al listado</Button></section>;
    return <TaxClassificationApiForm mode="edit" id={numericId} initialValues={valuesFromTaxClassificationDetail(detail)} />;
  }
