...
public sealed class AdminRolesEndpointBindingTests
{
    ...
    private sealed class CapturingAdministrationService : IAdministrationService
    {
        ...
        public Task<AdministrationResult<AdminUserResponse>> CreateUserAsync(string fullName, string email, string password, string? jobTitle, int roleId, int administratorId, string? originIp, CancellationToken cancellationToken) => Task.FromResult(AdministrationResult<AdminUserResponse>.Fail(404, "No implementado."));
