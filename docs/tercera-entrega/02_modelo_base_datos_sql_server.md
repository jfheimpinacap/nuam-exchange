...
## Prompt 032 — Revisión de alcance de Respaldo

Se inspeccionó la entidad real `BackupRecord`, mapeada a la tabla `Respaldo`. La tabla contiene `respaldo_id` como PK identity, `usuario_id` nullable como FK opcional a `Usuario`, `tipo_respaldo` requerido `nvarchar(50)`, `ruta_respaldo` requerida `nvarchar(600)`, `estado_respaldo` requerido `nvarchar(40)` con default `PROGRAMADO`, `fecha_respaldo` requerida `datetime2(0)` con default `SYSDATETIME()` y `observacion` nullable `nvarchar(700)`.
